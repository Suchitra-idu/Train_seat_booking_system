...
problem: what gets built, in what order, and the numbered decisions (D1–D19)
...
- Reserved e-tickets **sell out in ~42 seconds**; organized "agents" bulk-buy and
  resell (one Colombo ticket resold for **LKR 40,000**); the CID has an active
  investigation into the e-ticket racket.
...
non-overlapping legs raises effective reserved capacity, which simultaneously
attacks (a) empty-coach waste, (b) unfair short-leg pricing, and (c) the
artificial scarcity that feeds the tout economy. On top of that we add explicit
anti-hoarding controls. That is the story the README tells leadership.
...
|---|---|---|
| **D1** | **Stack:** FastAPI + SQLAlchemy 2.0 + Postgres (backend), React + Vite + TypeScript (frontend), Docker Compose. Tests: pytest + Hypothesis + Testcontainers + Playwright. | Closest to the user's Research Hexagon (import-linter enforceable), strong Postgres story for concurrency. Rejected TS-full-stack (less aligned) and Go (more boilerplate). |
...
| **D3** | **Unify both coach types on one mechanism: an unreserved coach is a reserved coach with hidden, auto-assigned seats.** N non-overlapping intervals across N virtual seats = per-leg capacity, enforced by the *same* `EXCLUDE` constraint. | One occupancy model, one correctness proof, one code path. The packing optimizer (D11) picks the virtual seat. Rejected a separate per-segment capacity-counter table (second concurrency mechanism to get right). |
...
| **D8** | **Identity:** no accounts/login. A booking records passenger name + NIC/passport and returns a reference. **Named-passenger-per-seat** is a policy (must match on board). | Removes anonymous-resale value (anti-tout). Rejected full auth (budget away from core) and fully-anonymous (breaks named-passenger policy). |
| **D9** | **Anti-tout controls:** per-passenger seat caps + booking velocity limits (config-driven); `AbuseScorer` port (heuristic now, **ML-ready**); idempotency keys; hold TTL; named-passenger policy. | Directly targets the 42-second-sellout racket. `AbuseScorer` keeps an ML seam without committing to a model. |
...
| **D11** | **Everything configurable:** number of coaches, seats/coach, coach class & type (reserved/unreserved), stations + km positions, fare rates, caps/limits, hold TTL — all from config/seed, **nothing hardcoded**. | Explicit challenge requirement (department may add coaches / extend the route). |
...
| **D13** | **Contract seam:** FastAPI's OpenAPI is the source of truth; `openapi-typescript` generates FE types. Contract tests both sides. | No hand-copied types; FE/BE drift becomes a red build. |
...
**Decisions to revisit** (sensible defaults set; flag if you disagree): unreserved
capacity caps at the coach's configured seat count (no standing overflow);
classes modeled as a config attribute on each coach with a fare multiplier;
currency is LKR; payment defaults to always-succeed with a test hook for decline.
...
Coach          id, code, type (RESERVED|UNRESERVED), class, seat_count   (from config)
Seat           id, coach_id, label, is_visible (false for unreserved virtual seats)
Passenger      id, name, nic_or_passport, email            (no account)
Booking        id, trip_id, seat_id, leg int4range '[)', status
               (HELD|CONFIRMED|CANCELLED|EXPIRED), passenger_id, reference,
               fare_cents, held_until, idempotency_key, created_at
...
frontend/
  ...
  adapters/   L2  api.real (from generated types) + api.fake · sse.real/fake → INTEGRATION
...
contract/     OpenAPI (from FastAPI) → generated TS types; contract tests both sides
```
...
|---|---|---|
| **P0 Rails** | repo layout, docker-compose (pg+api+web+migrate), Makefile (`check/lint/arch/test:unit/int/e2e/guard`), import-linter config, GitHub Actions, `.env.example`, OpenAPI→TS gen. `guard` proves enforcement is live. | arch + `guard` green |
...
| **P6 Frontend** | view-core (unit) → ports + fake client → real client from generated types + SSE → UI (route/seat/date pickers, **seat map** with per-leg availability colors, hold→confirm, optimistic + 409 UX, waitlist join) → app shell. | **UNIT** + **INTEGRATION** (fakes) + contract |
...
- **Gate:** `make check = typecheck + lint + import-linter + unit + integration`;
  CI runs it plus E2E on every push. Nothing merges red.
...
| **Anti-tout suite** (D9) | Scalper black market on reserved seats | Ella reserved e-tickets sell out in **~42 seconds**; a single Colombo ticket resold for up to **LKR 40,000**; active **CID investigation** into the racket | [R]/[B] |
