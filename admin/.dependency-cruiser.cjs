// The Dependency Rule for the admin frontend hexagon (same shape as web/, PLAN §3).
// Imports point inward only:  app > ui > adapters > ports > view-core.
// Layer imports are guarded here; the costly *globals* (fetch/localStorage) are guarded
// by eslint.config.js. Run: `depcruise src --config .dependency-cruiser.cjs`.
